...
export default function RecipeModal({ open, onOpenChange, recipe, foods, onSave }: RecipeModalProps) {
  ...
  return (
    <BottomSheet
      ...
      description="Monte a receita escolhendo alimentos e quantidades"
      className="max-h-[90vh]"
      dismissible={!saving}
      footer={
        <div className="shrink-0 border-t border-border bg-card p-4 pb-[calc(1rem+env(safe-area-inset-bottom,0px))]">
          <Button
            className="w-full"
            size="lg"
            onClick={handleSave}
          ...
          >
            {saving ? (
              <><Spinner size={16} className="text-primary-foreground" /> Salvando…</>
    ...
    >
      <div className="p-4 space-y-4 bottom-sheet-content">
        ...
        <div className="relative">
          ...
          {resultados.length > 0 && (
            <div className="fade-enter absolute left-0 right-0 top-full mt-1 z-20 rounded-lg border border-border bg-card shadow-lg max-h-48 overflow-y-auto">
              {resultados.map(food => (
                <button
                  key={food.id}
                  type="button"
                  className="pressable-row w-full text-left px-3 py-2 border-b border-border last:border-0"
        ...
        {itens.length > 0 && (
          <div className="space-y-2">
            ...
            {itens.map((item, i) => (
              <div key={item.draft_id} className="fade-enter flex items-center gap-2 rounded-lg border border-border bg-secondary/30 p-2">
                ...
                <button type="button" onClick={() => removeItem(i)} aria-label="Remover alimento" className="pressable-sm p-1 text-muted-foreground hover:text-destructive">
